...
attach(Boston)
#boston is a dataframe- so when you "attach"
#it really just attaches a "search term"

#this gets us the first demnsion. dim shows the 
#matrix form 506 rows by 14 variables
n = dim(Boston)[1]
...
#set the order in terms of x for the purpose of plotting
ind = order(test[,1])
#re-order the test
test =test[ind,]
...
for(i in kk){
        ...
        #kernal is the distance metric- so rectanglular is looking at Ecluidian 
        #distance so regular distance formulation
        
        #near has a whole lot of output. So we want to see how well we're doing-
        #so we compute our RMSE
...
#which position is the minimum? which.min
